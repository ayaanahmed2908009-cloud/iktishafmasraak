    ...
    <div className="min-h-[100dvh] bg-gray-50 dark:bg-[#18110B] p-6 lg:p-12">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Survey Responses</h1>
            ...
            <Table>
              <TableHeader className="bg-gray-100 dark:bg-[#1f160e]">
                <TableRow className="border-gray-200 dark:border-[#3A2A1A]">
                  <TableHead className="w-[100px] text-gray-600 dark:text-white">ID</TableHead>
                  <TableHead className="text-gray-600 dark:text-white">Name</TableHead>
                  <TableHead className="text-gray-600 dark:text-white">Phone</TableHead>
                  <TableHead className="text-gray-600 dark:text-white">Email</TableHead>
                  <TableHead className="text-gray-600 dark:text-white">Grade</TableHead>
                  <TableHead className="text-gray-600 dark:text-white">City</TableHead>
                  <TableHead className="text-gray-600 dark:text-white">School</TableHead>
                  <TableHead className="text-right text-gray-600 dark:text-white">Date</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {isLoading ? (
                  <TableRow>
                    <TableCell colSpan={8} className="text-center h-24 text-gray-500 dark:text-white">
                      Loading responses...
                    </TableCell>
                  </TableRow>
                  ...
                  responses.map((response) => (
                    <TableRow key={response.id} className="border-gray-200 dark:border-[#3A2A1A] hover:bg-gray-50 dark:hover:bg-[#2c2016]">
                      <TableCell className="font-medium dark:text-white">{response.id}</TableCell>
                      <TableCell className="dark:text-white">{response.name}</TableCell>
                      <TableCell className="dark:text-white">{response.phone}</TableCell>
                      <TableCell className="dark:text-white">{response.email}</TableCell>
                      <TableCell className="dark:text-white">{response.grade}</TableCell>
                      <TableCell className="dark:text-white">{response.city}</TableCell>
                      <TableCell className="dark:text-white">{response.school}</TableCell>
